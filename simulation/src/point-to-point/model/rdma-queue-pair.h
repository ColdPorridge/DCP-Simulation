...
public:
	...
	uint64_t m_size;
	uint64_t snd_nxt; // next seq to send, the highest unacked seq
	uint64_t m_mtu;
	bool m_finished = false;
	std::queue<uint32_t> m_retransQueue; // retransmission queue for HT packets
	uint16_t m_pg;
	uint16_t m_ipid;
	uint32_t m_win; // bound of on-the-fly packets
	uint32_t m_onfly; // on-the-fly packets number
	...
	void SetRetranTimeout(double retran_timeout);
