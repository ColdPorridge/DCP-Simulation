...
namespace ns3 {
	...
	RdmaEgressQueue::RdmaEgressQueue(){
		...
		m_ackQ->SetAttribute("MaxPackets", UintegerValue(0xffffffff)); // queue limit is on a higher level, not here
	}
	...
	int RdmaEgressQueue::GetNextQindex(bool paused[]){
		...
		for (qIndex = 1; qIndex <= fcount; qIndex++){
			...
			if (!paused[qp->m_pg] && qp->CanSend() > 0 && !qp->IsWinBound()){
	...
	uint32_t RdmaEgressQueue::GetNBytes(uint32_t qIndex){
		// not implemented
		NS_ASSERT_MSG(false, "RdmaEgressQueue::GetNBytes: not implemented");
		return 0;
	...
	void RdmaEgressQueue::RecoverQueue(uint32_t i){
		// not implemented
		NS_ASSERT_MSG(false, "RdmaEgressQueue::RecoverQueue: not implemented");
		...
		QbbNetDevice::DequeueAndTransmit(void)
	{
		...
		if (m_node->GetNodeType() == 0){
			int qIndex = m_rdmaEQ->GetNextQindex(m_paused);  // if -1, send a packet from high prio queue, else qIndex is the index of the qp, send a packet from that qp
			if (qIndex != -1024){
				if (qIndex == -1){ // high prio
				...
				Ptr<RdmaQueuePair> lastQp = m_rdmaEQ->GetQp(qIndex);  // here qIndex is actually the index of the qp
				p = m_rdmaEQ->DequeueQindex(qIndex);
