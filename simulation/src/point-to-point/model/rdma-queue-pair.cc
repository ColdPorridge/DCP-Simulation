...
#define HEADER_SIZE 48
 uint32_t RdmaRxQueuePair::bitmapSize = 0;
...
RdmaQueuePair::RdmaQueuePair(uint16_t pg, Ipv4Address _sip, Ipv4Address _dip, uint16_t _sport, uint16_t _dport, uint32_t qp_id){
...
void RdmaQueuePair::SetMtu(uint64_t mtu) {
	m_mtu = mtu;
}
...
void RdmaQueuePair::SetRetranTimeout(double retran_timeout){
	m_rto = Seconds(retran_timeout);
}
...
uint64_t RdmaQueuePair::GetBytesLeft(){
	uint64_t sent = snd_nxt * m_mtu;
	return m_size >= sent ? m_size - sent : 0;
}

bool RdmaQueuePair::CanSend(){
	return GetBytesLeft() > 0 || !m_retransQueue.empty();
}
...
void RdmaQueuePair::Acknowledge(uint64_t ack){
	NS_ASSERT_MSG(false, "RdmaQueuePair::Acknowledge: not implemented");
}

uint64_t RdmaQueuePair::GetOnTheFly(){
	return (m_onfly + 1) * (m_mtu + HEADER_SIZE);
}
...
uint32_t RdmaQueuePair::GetSeqPktSize(uint32_t seq) {
	uint64_t pkt_num = (m_size + m_mtu - 1) / m_mtu;
	NS_ASSERT(seq < pkt_num);
	if (seq == pkt_num - 1) {
		return m_size - (pkt_num - 1) * m_mtu;
	}
	return m_mtu;
}
...
bool RdmaQueuePair::IsFinished(uint32_t seq){
	if (!m_finished) {
		m_finished = seq  >= (m_size + m_mtu - 1) / m_mtu;
	}
	return m_finished;
}
