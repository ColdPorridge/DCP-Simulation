...
Time RdmaQueuePair::m_rto = Seconds(0.1);
...
bool RdmaQueuePair::HasPacket(){
	if (m_currentState == QpState::initial || m_currentState == QpState::normal)
		return m_win > (m_inflight + 1) && snd_nxt < (m_size + m_mtu - 1) / m_mtu;
	else
		return m_win > (m_inflight + 1) && snd_retx < (m_size + m_mtu - 1) / m_mtu;
}
...
bool RdmaQueuePair::IsWinBound(){
	NS_ASSERT_MSG(false, "IsWinBound is not implemented");
	uint64_t w = GetWin();
...
bool RdmaQueuePair::IsFinished(){
	return snd_una >= (m_size + m_mtu - 1) / m_mtu;
}
