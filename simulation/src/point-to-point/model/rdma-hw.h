...
public:
	...
	void AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address _sip, Ipv4Address _dip, uint16_t _sport, uint16_t _dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish, uint32_t qp_id, double retran_timeout); // add a new qp (new send)
	...
	int ReceiveAck(Ptr<Packet> p, CustomHeader &ch); // handle ACK
	int ReceiveNAck(Ptr<Packet> p, CustomHeader &ch); // handle NACK
	int ReceiveHT_dest(Ptr<Packet> p, CustomHeader &ch);
	int ReceiveHT_src(Ptr<Packet> p, CustomHeader &ch);
	...
	bool ReceiverCheckSeq(uint32_t &seq, Ptr<RdmaRxQueuePair> q, uint32_t size);
