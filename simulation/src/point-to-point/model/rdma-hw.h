...
public:
	static bool m_enableCC;
	static bool m_enablePathSelection;
	static uint64_t m_oooTolerance;
	static bool m_enableRecovery;
	static uint32_t vp_seed;
	...
	void AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address _sip, Ipv4Address _dip, uint16_t _sport, uint16_t _dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish, uint32_t qp_id); // add a new qp (new send)
	...
	int ReceiveAck(Ptr<Packet> p, CustomHeader &ch); // handle ACK and NACK
	...
	bool ReceiverCheckSeq(uint32_t seq, Ptr<RdmaRxQueuePair> q, uint32_t &ePSN);
	 ...
	 * MPRDMA
	 ***********************/
	void MpRDMARecvAckLogic(Ptr<RdmaQueuePair> qp, uint32_t seq, uint32_t epsn, uint8_t cnp, uint16_t vp, bool isNack, bool isReTx);
	void SenderTimeout(Ptr<RdmaQueuePair> qp);
	TracedCallback<Ptr<RdmaQueuePair>> m_traceTimeout; // trace for timeout
	/***********************
	 * High Precision CC
	 ***********************/
	double m_targetUtil;
