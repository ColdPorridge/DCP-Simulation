...
bool RdmaHw::m_ResponseHT = false;
...
void RdmaHw::AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish, uint32_t qp_id, double retran_timeout){
	// create qp
	...
	qp->SetMtu(m_mtu);
	qp->SetRetranTimeout(retran_timeout);
	// add qp
...
int RdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader &ch){
	...
	uint32_t new_ePSN = ch.udp.seq;
	bool valid = ReceiverCheckSeq(new_ePSN, rxQp, payload_size);

	if (valid){ //generate ACK
		qbbHeader seqh;
		seqh.SetSeq(new_ePSN);
		seqh.SetPG(0);
		...
		newp->AddPacketTag(OriginPgTag(ch.udp.pg));

		// send
		uint32_t nic_idx = GetNicIdxOfRxQp(rxQp);
		m_nic[nic_idx].dev->RdmaEnqueueHighPrioQ(newp);
		m_nic[nic_idx].dev->TriggerTransmit();
	} else { // out of bitmap: generate HT
		qbbHeader seqh;
		...
		head.SetDestination(Ipv4Address(ch.sip));
		head.SetSource(Ipv4Address(ch.dip));
		head.SetProtocol(0xF1); //ack=0xFC nack=0xFD HT2dest=0xF0 HT2src=0xF1
		head.SetTtl(64);
		head.SetPayloadSize(newp->GetSize());
...
void RdmaHw::SenderTimeout(Ptr<RdmaQueuePair> qp){
	qp->snd_nxt = 0;
	qp->m_onfly = 0;
	while (!qp->m_retransQueue.empty()) {
		qp->m_retransQueue.pop();
	}
...
int RdmaHw::ReceiveAck(Ptr<Packet> p, CustomHeader &ch){
	...
	uint32_t nic_idx = GetNicIdxOfQp(qp);
	Ptr<QbbNetDevice> dev = m_nic[nic_idx].dev;
	NS_ASSERT_MSG(qp->m_onfly > 0, "ACK received but no packet on fly");
	qp->m_onfly -= 1;
	if (qp->IsFinished(seq)){
		QpComplete(qp);
	}
...
int RdmaHw::ReceiveNAck(Ptr<Packet> p, CustomHeader &ch){
	NS_ASSERT_MSG(false, "NACK not implemented");
	return 0;
}
...
int RdmaHw::Receive(Ptr<Packet> p, CustomHeader &ch){
	if (ch.l3Prot == 0x11){ // UDP
		...
		ReceiveAck(p, ch);
	} else if (ch.l3Prot == 0xF0){ // HT-dest
		ReceiveHT_dest(p, ch);
	} else if (ch.l3Prot == 0xF1){ // HT-src
		ReceiveHT_src(p, ch);
	}
	return 0;
}

int RdmaHw::ReceiveHT_dest(Ptr<Packet> p, CustomHeader &ch) {
	uint8_t ecnbits = ch.GetIpv4EcnBits();
	OriginPgTag t;
	p->PeekPacketTag(t);
	uint16_t origin_pg = t.GetPG();
	Ptr<RdmaRxQueuePair> rxQp = GetRxQp(ch.dip, ch.sip, ch.udp.dport, ch.udp.sport, origin_pg, true);
	qbbHeader seqh;
	seqh.SetSeq(ch.udp.seq);
	seqh.SetPG(0);
	seqh.SetSport(ch.udp.dport);
	...
	head.SetDestination(Ipv4Address(ch.sip));
	head.SetSource(Ipv4Address(ch.dip));
	head.SetProtocol(0xF1); //ack=0xFC nack=0xFD
	head.SetTtl(64);
	head.SetPayloadSize(newp->GetSize());
	...
	newp->AddPacketTag(OriginPgTag(origin_pg));
	// send
	uint32_t nic_idx = GetNicIdxOfRxQp(rxQp);
	m_nic[nic_idx].dev->RdmaEnqueueHighPrioQ(newp);
	m_nic[nic_idx].dev->TriggerTransmit();
...
int RdmaHw::ReceiveHT_src(Ptr<Packet> p, CustomHeader &ch) {
	OriginPgTag t;
	p->PeekPacketTag(t);
	uint16_t origin_pg = t.GetPG();
	uint8_t cnp = (ch.ack.flags >> qbbHeader::FLAG_CNP) & 1;
	Ptr<RdmaQueuePair> qp = GetQp(ch.sip, ch.ack.dport, origin_pg);
	NS_ASSERT(qp != NULL);
	qp->m_retransQueue.push(ch.udp.seq);
	NS_ASSERT_MSG(qp->m_onfly > 0, "HT received but no packet on fly");
	qp->m_onfly -= 1;

	if (RdmaHw::m_ResponseHT){
...
bool RdmaHw::ReceiverCheckSeq(uint32_t &seq, Ptr<RdmaRxQueuePair> q, uint32_t size){
	uint32_t expected = q->ReceiverNextExpectedSeq;
	size_t ePSN_delta = 0;
	bool valid = q->m_bitmap->writeBit(seq - expected, ePSN_delta);
	q->ReceiverNextExpectedSeq += ePSN_delta;
	seq = valid ? expected + ePSN_delta : 0;
	return valid;
}
...
void RdmaHw::RecoverQueue(Ptr<RdmaQueuePair> qp){
	NS_ASSERT_MSG(false, "RdmaHw::RecoverQueue not implemented");
}
...
Ptr<Packet> RdmaHw::GetNxtPacket(Ptr<RdmaQueuePair> qp){
	...
	if (qp->m_retransQueue.empty()) {
		seq = qp->snd_nxt;
		qp->snd_nxt += 1;
	} else {
		seq = qp->m_retransQueue.front();
		qp->m_retransQueue.pop();
		qp->m_retx_num++;
	}
	qp->m_onfly += 1;
