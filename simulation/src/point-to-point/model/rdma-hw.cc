...
#include "cn-header.h"
#include <random>
#include "ns3/vp-tag.h"
#include "ns3/origin-pg-tag.h"
#include "ns3/epsn-tag.h"
#include "ns3/retx-tag.h"
namespace ns3{
bool RdmaHw::m_enableCC = true;
bool RdmaHw::m_enablePathSelection = true;
uint64_t RdmaHw::m_oooTolerance = 32;
bool RdmaHw::m_enableRecovery = true;
uint32_t RdmaHw::vp_seed = 42;
TypeId RdmaHw::GetTypeId (void)
{
	static TypeId tid = TypeId ("ns3::RdmaHw")
		...
		.AddAttribute("PintSmplThresh",
				...
				MakeUintegerChecker<uint32_t>())
		.AddTraceSource ("Timeout", "get a qp",
					MakeTraceSourceAccessor (&RdmaHw::m_traceTimeout))
		;
	return tid;
}
...
void RdmaHw::AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish, uint32_t qp_id){
	// create qp
	Ptr<RdmaQueuePair> qp = CreateObject<RdmaQueuePair>(pg, sip, dip, sport, dport, qp_id);
	qp->SetSize(size);
	qp->SetWin(win);
	...
	qp->SetMtu(m_mtu);
	qp->m_currentState = QpState::initial;
	qp->m_inflight = 0;
	qp->m_initialWin = win;
	qp->snd_ooh = 0;
	// add qp
...
Ptr<RdmaRxQueuePair> RdmaHw::GetRxQp(uint32_t sip, uint32_t dip, uint16_t sport, uint16_t dport, uint16_t pg, bool create){
	...
	if (create){
		...
		q->sport = sport;
		q->dport = dport;
		q->m_bitmap = CreateObject<RingBitmap>(RdmaRxQueuePair::bitmapSize);
...
int RdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader &ch){
	...
	uint32_t new_ePSN = rxQp->ReceiverNextExpectedSeq;
	bool valid = ReceiverCheckSeq(ch.udp.seq, rxQp, new_ePSN);
	rxQp->ReceiverNextExpectedSeq = new_ePSN;
	
	qbbHeader seqh;
	seqh.SetSeq(ch.udp.seq); // SACK field
	seqh.SetPG(0);
	...
	newp->AddPacketTag(EpsnTag(new_ePSN)); // add tag for epsn(AACK field)
	
	newp->AddPacketTag(OriginPgTag(ch.udp.pg)); // add tag for origin pg
	
	VpTag vp_tag;
	p->PeekPacketTag(vp_tag);
	uint16_t vp = vp_tag.GetVp();
	newp->AddPacketTag(VpTag(vp)); // add tag for vp

	ReTxTag retx_tag;
	p->PeekPacketTag(retx_tag);
	uint32_t retx = retx_tag.GetReTx();
	NS_ASSERT_MSG(retx == 0 || retx == 1, "retx should be 0 or 1");
	newp->AddPacketTag(ReTxTag(retx)); // add tag for retx

	Ipv4Header head;	// Prepare IPv4 header
	head.SetDestination(Ipv4Address(ch.sip));
	head.SetSource(Ipv4Address(ch.dip));
	head.SetProtocol(valid ? 0xFC : 0xFD); //ack=0xFC nack=0xFD
	head.SetTtl(64);
...
void RdmaHw::SenderTimeout(Ptr<RdmaQueuePair> qp){
	m_traceTimeout(qp);
	qp->m_timeOutNum++;
	qp->snd_nxt = qp->snd_una;
	qp->m_inflight = 0;
	qp->m_currentState = QpState::initial;
	qp->m_win = qp->m_initialWin;
	qp->m_eventTimeout = Simulator::Schedule(qp->m_rto, &RdmaHw::SenderTimeout, this, qp);
	qp->snd_ooh = 0;
...
int RdmaHw::ReceiveAck(Ptr<Packet> p, CustomHeader &ch){
	OriginPgTag t;
	p->PeekPacketTag(t);
	uint16_t qIndex = t.GetPG();
	uint16_t port = ch.ack.dport;
	uint32_t seq = ch.ack.seq;
	uint8_t cnp = (ch.ack.flags >> qbbHeader::FLAG_CNP) & 1;
	bool isNack = (ch.l3Prot == 0xFD);
	int i;
	Ptr<RdmaQueuePair> qp = GetQp(ch.sip, port, qIndex);
	if (qp == NULL){
		// QP already finished and deleted
		return 0;
	}
	uint32_t nic_idx = GetNicIdxOfQp(qp);

	EpsnTag ePSN_tag;
	p->PeekPacketTag(ePSN_tag);
	uint32_t ePSN = ePSN_tag.GetSeq();
	...
	Ptr<QbbNetDevice> dev = m_nic[nic_idx].dev;
	MpRDMARecvAckLogic(qp, seq, ePSN, cnp, vp, RdmaHw::m_enableRecovery && isNack, retx);
	if (qp->IsFinished()){
		QpComplete(qp);
	}
...
void RdmaHw::MpRDMARecvAckLogic(Ptr<RdmaQueuePair> qp, uint32_t seq, uint32_t epsn, uint8_t cnp, uint16_t vp, bool isNack, bool isReTx) {
	// update inflight
	qp->m_inflight = std::max(qp->m_inflight, 1U) - 1U;
	NS_ASSERT_MSG(qp->m_inflight >= 0, "inflight should be greater than 0");
	// una move 
	if (epsn > qp->snd_una){
		qp->snd_una = epsn;
		qp->snd_nxt = std::max(qp->snd_nxt, qp->snd_una);
		Simulator::Cancel(qp->m_eventTimeout);
		qp->m_eventTimeout = Simulator::Schedule(qp->m_rto, &RdmaHw::SenderTimeout, this, qp);
	}
	// CC logic
	if (m_enableCC)
		qp->m_win = std::max(1.0, cnp ? qp->m_win - 0.5 : qp->m_win + 1 / qp->m_win);
	// state transfer
	if (qp->m_currentState == QpState::initial && !isNack){
		qp->m_currentState = QpState::normal;
	} else if (qp->m_currentState == QpState::normal && isNack){
		qp->m_currentState = QpState::recovery;
		qp->snd_retx = qp->snd_una;
	} else if (qp->m_currentState == QpState::recovery && qp->snd_una >= qp->snd_nxt){
		qp->m_currentState = QpState::normal;
	}
	// ooo-aware path selection
	if (!m_enablePathSelection){
		qp->m_currentVp = vp;
	} else if (!isNack && !isReTx) {
		uint32_t snd_ool = std::max(qp->snd_ooh, RdmaHw::m_oooTolerance) - RdmaHw::m_oooTolerance;
		if (seq >= qp->snd_ooh) {
			qp->snd_ooh = seq;
			qp->m_currentVp = vp;
		} else if (seq <= snd_ool) {
			qp->m_win =  m_enableCC ? std::max(1.0, qp->m_win - 1) : qp->m_win;
		} else {
			qp->m_currentVp = vp;
		}
	} else {
...
int RdmaHw::Receive(Ptr<Packet> p, CustomHeader &ch){
	if (ch.l3Prot == 0x11){ // UDP
		...
		ReceiveCnp(p, ch);
	}else if (ch.l3Prot == 0xFC || ch.l3Prot == 0xFD){ // ACK
		ReceiveAck(p, ch);
	}
	return 0;
}

bool RdmaHw::ReceiverCheckSeq(uint32_t seq, Ptr<RdmaRxQueuePair> q, uint32_t &ePSN){
	ePSN = q->ReceiverNextExpectedSeq;
	if (seq < ePSN){
		// no need to update ePSN, should send ACK back
		return true;
	}
	size_t ePSN_delta = 0;
	bool valid = q->m_bitmap->writeBit(seq - ePSN, ePSN_delta);
	ePSN += ePSN_delta;
	return valid;
}
...
uint16_t generateRandomUInt16() {
	std::mt19937 gen(RdmaHw::vp_seed++);
...
Ptr<Packet> RdmaHw::GetNxtPacket(Ptr<RdmaQueuePair> qp){
	...
	uint16_t vp;
	uint32_t isReTx = 0;
	if (qp->m_currentState == QpState::initial) {
		seq = qp->snd_nxt;
		qp->snd_nxt += 1;
		vp = generateRandomUInt16();
	} else if (qp->m_currentState == QpState::normal) {
		seq = qp->snd_nxt;
		qp->snd_nxt += 1;
		vp = qp->m_currentVp;
	} else {
	...
	qp->m_inflight += 1;
	NS_ASSERT_MSG(qp->m_inflight <= qp->m_win, "inflight should be less than win");
	// start time out timer
	if (!qp->m_start) {
		qp->m_start = true;
		qp->m_eventTimeout = Simulator::Schedule(qp->m_rto, &RdmaHw::SenderTimeout, this, qp);
	}
	payload_size = qp->GetSeqPktSize(seq);
	...
	p->AddHeader (ppp);
	// add ReTxTag
	p->AddPacketTag(ReTxTag(isReTx));
	p->AddPacketTag(VpTag(vp));
