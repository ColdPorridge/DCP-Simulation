...
int SwitchNode::GetOutDevRps(Ptr<const Packet> p, CustomHeader &ch){
	// look up entries
	auto entry = m_rtTable.find(ch.dip);
	...
	uint32_t idx = 0; 
	if (nexthops.size() > 1)
		idx = generateRandomUInt16() % nexthops.size();
	return nexthops[idx];
}
...
void SwitchNode::SendToDev(Ptr<Packet>p, CustomHeader &ch){
	...
	else if (m_routeMode == 2)
		idx = GetOutDevRps(p, ch);
	else
...
Ptr<Packet> SwitchNode::headerTrim(CustomHeader &ch) {
	qbbHeader seqh;
	seqh.SetSeq(ch.udp.seq);
	seqh.SetPG(0);
	seqh.SetSport(ch.udp.sport);
	seqh.SetDport(ch.udp.dport);
	Ptr<Packet> ht_packet = Create<Packet>(std::max(60-14-20-(int)seqh.GetSerializedSize(), 0));
	ht_packet->AddHeader(seqh);

	Ipv4Header head;
	head.SetDestination(Ipv4Address(ch.dip));
	head.SetSource(Ipv4Address(ch.sip));
	head.SetProtocol(0xF0);                 // 0xF0 for HT to dest, 0xF1 for HT to src
	head.SetTtl(64);
	head.SetPayloadSize(ht_packet->GetSize());
	head.SetEcn((Ipv4Header::EcnType)0x03);
	ht_packet->AddHeader(head);

	PppHeader ppp;
	ppp.SetProtocol(0x0021);
	ht_packet->AddHeader(ppp);
	ht_packet->AddPacketTag(OriginPgTag(ch.udp.pg));
	return ht_packet;
}
