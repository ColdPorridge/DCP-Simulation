...
RdmaClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::RdmaClient")
    ...
    .AddAttribute ("QpId",
                    "Queue pair ID",
                    UintegerValue (0),
                    MakeUintegerAccessor (&RdmaClient::qp_id),
                    MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("RetranTimeout",
                    "Retransmission timeout",
                    DoubleValue (0.008),  // s
                    MakeDoubleAccessor (&RdmaClient::retran_timeout),
...
void RdmaClient::StartApplication (void)
{
  ...
  rdma->AddQueuePair(m_size, m_pg, m_sip, m_dip, m_sport, m_dport, m_win, m_baseRtt, MakeCallback(&RdmaClient::Finish, this), qp_id, retran_timeout);
}
