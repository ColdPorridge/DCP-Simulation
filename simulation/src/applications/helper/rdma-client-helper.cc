...
RdmaClientHelper::RdmaClientHelper (uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint64_t size, uint32_t win, uint64_t baseRtt, uint32_t qp_id, double retran_timeout)
{
  ...
  SetAttribute ("QpId", UintegerValue (qp_id));
  SetAttribute ("RetranTimeout", DoubleValue (retran_timeout));
}
