...
void qp_finish(FILE* fout, Ptr<RdmaQueuePair> q){
    ...
    fprintf(fout, "%u %u %u %u %lu %lu %lu %lu %lu %u %u\n",
        ...
        q->m_retx_num * packet_payload_size, q->m_timeout_count, q->m_qpId);
    fflush(fout);
...
int main(int argc, char *argv[])
{
...
#endif
	{
		...
		while (!conf.eof())
		{
			...
			if (key.compare("ENABLE_QCN") == 0)
			{
				...
				conf >> v;
				retran_timeout = v;
				std::cout << "RETRAN_TIMEOUT\t\t\t" << retran_timeout << "\n";
	...
	std::cout << "Finish reading config file\n";
